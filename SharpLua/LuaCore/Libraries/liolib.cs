...
namespace SharpLua
{
    ...
    public partial class Lua
    {
        ...
        public static FilePtr tofilep(LuaState L) { return (FilePtr)luaL_checkudata(L, 1, LUA_FILEHANDLE); }


        private static int io_type (LuaState L) {
            object ud;
            luaL_checkany(L, 1);
         ...
         */
        private static int io_noclose (LuaState L) {
            lua_pushnil(L);
         ...
         */
        private static int io_pclose (LuaState L) {
            FilePtr p = tofilep(L);
         ...
         */
        private static int io_fclose (LuaState L) {
            FilePtr p = tofilep(L);
        ...
        private static int aux_close (LuaState L) {
            lua_getfenv(L, 1);
        ...
        private static int io_close (LuaState L) {
            if (lua_isnone(L, 1))
        ...
        private static int io_gc (LuaState L) {
        ...
        private static int io_tostring (LuaState L) {
            Stream f = tofilep(L).file;
            if (f == null)
        ...
        private static int io_open (LuaState L) {
         ...
         */
        private static int io_popen (LuaState L) {
            CharPtr filename = luaL_checkstring(L, 1);
            CharPtr mode = luaL_optstring(L, 2, "r");
            FilePtr pf = newfile(L);
        ...
        private static int io_tmpfile (LuaState L) {
            FilePtr pf = newfile(L);
            #if XBOX
        ...
        private static int g_iofile (LuaState L, int f, CharPtr mode) {
        ...
        private static int io_input (LuaState L) {
            return g_iofile(L, IO_INPUT, "r");
        }


        private static int io_output (LuaState L) {
            return g_iofile(L, IO_OUTPUT, "w");
        }

        private static void aux_lines (LuaState L, int idx, int toclose) {
            lua_pushvalue(L, idx);
        ...
        private static int f_lines (LuaState L) {
            tofile(L);  /* check that it's a valid file file */
            aux_lines(L, 1, 0);
            return 1;
        }
        ...
        private static int io_lines (LuaState L) {
        ...
        private static int test_eof (LuaState L, Stream f) {
            int c = getc(f);
            ungetc(c, f);
        ...
        private static int read_line (LuaState L, Stream f) {
        ...
        private static int read_chars (LuaState L, Stream f, uint n) {
        ...
        private static int g_read (LuaState L, Stream f, int first) {
        ...
        private static int io_read (LuaState L) {
            return g_read(L, getiofile(L, IO_INPUT), 1);
        }


        private static int f_read (LuaState L) {
            return g_read(L, tofile(L), 2);
        }


        private static int io_readline (LuaState L) {
        ...
        private static int g_write (LuaState L, Stream f, int arg) {
            int nargs = lua_gettop(L) - 1;
            int status = 1;
            for (; (nargs--) != 0; arg++) {
        ...
        private static int io_write (LuaState L) {
            return g_write(L, getiofile(L, IO_OUTPUT), 1);
        }


        private static int f_write (LuaState L) {
            return g_write(L, tofile(L), 2);
        }

        

        private static int f_seek (LuaState L) {
        ...
        private static int f_setvbuf (LuaState L) {
        ...
        private static int io_flush (LuaState L) {
            int result = 1;
        ...
        private static int f_flush (LuaState L) {
            int result = 1;
        ...
        private static void newfenv (LuaState L, lua_CFunction cls) {
        ...
        public static int luaopen_io (LuaState L) {
            createmeta(L);
